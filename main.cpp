...
{
	FrameBuffer   * eyeRenderTexture[2] = { nullptr, nullptr };
    ...
    for (int eye = 0; eye < 2; ++eye)
    {
        ...
        eyeRenderTexture[eye] = new FrameBuffer(HMD, true, true, idealTextureSize, 1, NULL, 1);
    ...
    while (Platform.HandleMessages())
    {
		...
		if(roomScene->numModels != 0){
			roomScene->Models[0]->Pos = Vector3f(9 * sin(cubeClock), 3, 9 * cos(cubeClock += 0.015f));
		}

		static float isoClock = 0;
		static float volumelock = 0;

		if (roomScene->numVolumeModels != 0) {			
			//roomScene->volumeModels[0]->Rot = Quatf(Vector3f(0.0, 1.0, 0.0), sin(volumelock += 0.015f));
			//roomScene->volumeModels[0]->iso_value = abs(sin(isoClock += 0.003f));
		}

		for (int m = 0; m != roomScene->numVolumeModels; ++m) {
			//roomScene->volumeModels[m]->iso_value = abs(sin(isoClock));
		}
