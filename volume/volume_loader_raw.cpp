...
glm::ivec3 Volume_loader_raw::get_dimensions(const std::string filepath) const
{
  ...
  p1 = filepath.rfind("_h");
  p0 = filepath.rfind("_w", p1) + 2;
  ...
  p1 = filepath.rfind("_d");
  p0 = filepath.rfind("_h", p1) + 2;
  ...
  p1 = filepath.rfind("_c");
  p0 = filepath.rfind("_d", p1) + 2;
...
unsigned Volume_loader_raw::get_channel_count(const std::string filepath) const
{
  ...
  size_t p0 = 0, p1 = std::string::npos, t0 = 0;

  p1 = filepath.rfind("_b");
  p0 = filepath.rfind("_c", p1) + 2;

  if (p0 == filepath.front()){
      channels = 1;
  }
  else{
    std::string token = filepath.substr(p0, p1 - p0);
    channels = std::max(std::atoi(token.c_str()), 1);
  }
